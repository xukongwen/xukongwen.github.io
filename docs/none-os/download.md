...
[0.004b版2021.2.15](https://pan.baidu.com/s/1u16dJevET7V_P5t5-97-Xw)
...
这个版本的变化非常大。增加和修改了很多东西。

- 彻底选择了Vim，我很爱Emacs，只是在Pi Zero上太慢了。

- 加入了所谓主菜单，可以方便导航到最近的文件和一些设置文档，当然也有一些正在制作的帮助文档。

- 加入了fzf搜索，这简直是神器！但配置的还不好，未来希望可以好好配置。

- 将fcitx默认拼音改成了rime，非常好用。

- 加入了davf2这个webdav程序，可以方便的将文件和手机或者电脑同步，比如用坚果云。

- 加入了zsh，以及o-my-zsh，稍微配置了一些插件。

- 在vim中加入了大量leader快捷键，基本上是按空格，然后激活各种功能，相关帮助还在写。
...
这个版本将Vim作为默认编辑器，原因也很简单，测试发现Emacs对于超过70万字的文档几乎无法操作（严重延时），同时打开文档的速度，Vim大约是Emacs的3-10倍。而Vim打开100万字文档几乎秒开，而且输入没有任何延迟。
...
None-OS可以在任意树莓派上面跑，但我建议在Pi Zero上面尝试。

Pi Zero 支持1080p输出，开机到进入系统为1分中左右，在Vim内操作几乎无卡顿。（打开100万字的文档要2-4秒）

Pi 4 支持4K60fps输出，开机约为30秒，操作流畅。（打开100万字文档在2秒左右）
...
[脚本](http:/none-os.com/docs/none-os/pishrink.sh)
...
基本上就大功告成了。（根据情况不同，压缩率也不同，我大概是15G可压缩到15.-3G左右。随后你还可以zip一下，体积可以继续缩减，期待未来可以减肥）

整个过程基本上就是这样了，期待高手指正和优化其中不好的部分。
